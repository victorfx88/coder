...
func TestDotfiles(t *testing.T) {
        ...
        t.Run("InstallScript", func(t *testing.T) {
                ...
                if runtime.GOOS == "windows" {
                        ...
                        scriptContent = fmt.Sprintf("Set-Content -Path '%s' -Value 'wow'", filepath.Join(string(root), ".bashrc"))
                } else {
                        scriptName = "install.sh"
                        scriptContent = "#!/bin/bash\necho wow > " + filepath.Join(string(root), ".bashrc")
                }

                // nolint:gosec
                err := os.WriteFile(filepath.Join(testRepo, scriptName), []byte(scriptContent), 0o750)
                require.NoError(t, err)

                c := exec.Command("git", "add", scriptName)
                c.Dir = testRepo
                err = c.Run()
                require.NoError(t, err)

                c = exec.Command("git", "commit", "-m", fmt.Sprintf(`"add %s"`, scriptName))
                c.Dir = testRepo
                err = c.Run()
                require.NoError(t, err)
        ...
        t.Run("CrossPlatformScripts", func(t *testing.T) {
                t.Parallel()
                _, root := clitest.New(t)
                testRepo := testGitRepo(t, root)

                // Create both Windows and Unix scripts
                // nolint:gosec
                err := os.WriteFile(filepath.Join(testRepo, "install.ps1"), 
                        []byte(fmt.Sprintf("Set-Content -Path '%s' -Value 'windows'", filepath.Join(string(root), ".config"))), 0o750)
                require.NoError(t, err)

                // nolint:gosec
                err = os.WriteFile(filepath.Join(testRepo, "install.sh"),
                        []byte("#!/bin/bash\necho unix > "+filepath.Join(string(root), ".config")), 0o750)
                require.NoError(t, err)
                ...
                b, err := os.ReadFile(filepath.Join(string(root), ".config"))
                require.NoError(t, err)
                if runtime.GOOS == "windows" {
                        require.Equal(t, string(b), "windows")
                } else {
                        require.Equal(t, string(b), "unix\n")
                }
        })
        ...
        t.Run("NestedInstallScript", func(t *testing.T) {
                ...
                if runtime.GOOS == "windows" {
                        scriptPath = filepath.Join("script", "setup.ps1")
                        scriptContent = fmt.Sprintf("Set-Content -Path '%s' -Value 'wow'", filepath.Join(string(root), ".bashrc"))
                } else {
                ...
                require.NoError(t, err)
                // nolint:gosec
                err = os.WriteFile(filepath.Join(testRepo, scriptPath), []byte(scriptContent), 0o750)
                require.NoError(t, err)
