        ...
        cmd := &serpent.Command{
                ...
                Handler: func(inv *serpent.Invocation) error {
                        var (
                                ...
                                windowsScriptSet = []string{
                                        "install.ps1",
                                        "setup.ps1",
                                        "bootstrap.ps1",
                                },
                                // Unix-like scripts (Linux/macOS)
                                unixScriptSet = []string{
                                        "install.sh",
                                        "install",
                                        "bootstrap.sh",
                                        "bootstrap",
                        ...
                        // Check for Windows-specific scripts first
                        script := findScript(windowsScriptSet, dotfilesDir)
                        isWindows := false
                        if script != "" {
                                isWindows = true
                        } else {
                                // If no Windows script found, try Unix scripts
                                script = findScript(unixScriptSet, dotfilesDir)
                        }
                        ...
                        if script != "" {
                                ...
                                if isWindows {
                                        ...
                                        scriptCmd = exec.CommandContext(inv.Context(), "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", scriptPath)
                                } else {
                                        // Check if the script is executable and notify on error for Unix scripts
                                        if fi.Mode()&0o111 == 0 {
                                                return xerrors.Errorf("script %q does not have execute permissions", script)
                                        }
                                        // Execute shell script on Unix-like systems
