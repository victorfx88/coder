...
`,
			...
			Schema: aisdk.Schema{
				Properties: map[string]any{
					"owner": map[string]any{
						"type":        "string",
						"description": "The owner of the workspaces to list. Use \"me\" to list workspaces for the authenticated user.",
					},
				},
			},
		},
	...
	ListTemplates = Tool[[]MinimalTemplate]{
		...
		Handler: func(ctx context.Context, args map[string]any) ([]MinimalTemplate, error) {
			client, err := clientFromContext(ctx)
			if err != nil {
				return nil, err
			}
	...
	GetAuthenticatedUser = Tool[codersdk.User]{
		...
		Handler: func(ctx context.Context, args map[string]any) (codersdk.User, error) {
			client, err := clientFromContext(ctx)
			if err != nil {
...
`,
		...
		Handler: func(ctx context.Context, args map[string]any) (codersdk.TemplateVersion, error) {
			...
			fileID, err := uuid.Parse(args["file_id"].(string))
			if err != nil {
				return codersdk.TemplateVersion{}, err
			}
			var templateID uuid.UUID
			if args["template_id"] != nil {
				templateID, err = uuid.Parse(args["template_id"].(string))
				if err != nil {
					return codersdk.TemplateVersion{}, err
				}
			}
...
More logs may appear after this call. It does not wait for the agent to finish.`,
		...
		Handler: func(ctx context.Context, args map[string]any) ([]string, error) {
			...
			workspaceAgentID, err := uuid.Parse(args["workspace_agent_id"].(string))
			if err != nil {
				return nil, err
			}
...
Useful for checking whether a workspace builds successfully or not.`,
		...
		Handler: func(ctx context.Context, args map[string]any) ([]string, error) {
			...
			workspaceBuildID, err := uuid.Parse(args["workspace_build_id"].(string))
			if err != nil {
				return nil, err
			}
		...
		Handler: func(ctx context.Context, args map[string]any) (codersdk.UploadResponse, error) {
			...
			go func() {
				...
				for name, content := range files {
					...
					if _, err := tarWriter.Write([]byte(content.(string))); err != nil {
						pipeWriter.CloseWithError(err)
						return
					}
				}
	...
	CreateTemplate = Tool[codersdk.Template]{
		...
		Handler: func(ctx context.Context, args map[string]any) (codersdk.Template, error) {
			...
			versionID, err := uuid.Parse(args["version_id"].(string))
			if err != nil {
				return codersdk.Template{}, err
			}
